        ...
        navigator.mediaDevices.getUserMedia({ audio: true, video: false })
            .then(stream => {
                ...
                mediaRecorder.onstop = () => {
                    ...
                    let audioFile = new File([audioBlob], "audio.ogg", { type: 'audio/ogg' });
                    
                    /*
                    theForm = new FormData(audioForm)
                    theForm.append("live_recording", audioFile)
                    console.log(theForm)
                    

                    audioInput.type = 'file'; // Using hidden type to avoid displaying the input field
                    audioInput.name = 'live_recording';
                    audioInput.files = new DataTransfer().items.add(audioFile).files;
                    audioForm.appendChild(audioInput);

                    let theForm = new FormData(audioForm);
                    fetch("{{ url_for('convert_wav') }}", {
                        method: 'POST',
                        body: theForm
                    }).then(response => {
